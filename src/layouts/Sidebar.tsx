...
const SidebarItem = ({ icon, href, label, onClick }: SidebarItemProps) => {
  ...
  const activeStyle = !onClick && href === pathname && "bg-primary text-white";
  ...
  return (
    ...
    >
      {icon}
      <span className="text-large font-medium">{label}</span>
    </Link>
  );
};
      ...
      {authSnap.profile && (
        <div className="flex flex-col items-center gap-2 mt-12 mb-4">
          <img
            ...
            alt="avatar"
          />
          <p className="text-tertiary text-xl font-bold">
